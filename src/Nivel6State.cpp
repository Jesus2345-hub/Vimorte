      ...
      m_bloquearR(false)
{
    ...
    m_gallo.setLimites(1350.f, 1600.f);
    m_cercaGallo = false;
    
    // Joven
    m_joven.setPosition(303.f, 288.f);
    m_joven.setScale(0.15f, 0.15f);
    m_cercaJoven = false;
    
    // Abuelita
    m_abuelita.setPosition(1535.f, 327.f);
    m_abuelita.setScale(0.15f, 0.15f);
    ...
    if (const auto* keyPressed = event.getIf<sf::Event::KeyPressed>()) {
        ...
        if (keyPressed->code == sf::Keyboard::Key::M) {
            if (game->tienePartidaActiva()) {
                const auto& items = game->getSaveManager().getCurrentProgress().itemsRecolectados;
                auto it = std::find(items.begin(), items.end(), "TutorialNivel6Visto");
                if (it != items.end()) m_mostrarTutorialPorTecla = true;
                else m_mostrarTutorial = true;
            }
        }
    }
...
void Nivel6State::update(float dt) 
{
    ...
    if (!m_rifleRecogido) {
        ...
        if (m_cercaRifle && sf::Keyboard::isKeyPressed(sf::Keyboard::Key::R)) {
            if (!rRiflePresionado) { rRiflePresionado = true; m_rifleRecogido = true; mostrarMensaje("Rifle recogido!", 2.f, sf::Color::Green); }
        } else rRiflePresionado = false;
    }
    ...
    if (m_rifleRecogido && m_cercaGallo && !m_roosterHuntMinigame.isActive() && !m_tictactoeMinigame.isActive()) {
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::R)) {
            if (!rGalloPresionado) { rGalloPresionado = true; m_roosterHuntMinigame.activate(); }
        } else rGalloPresionado = false;
    }
    ...
    if (m_roosterHuntMinigame.isActive()) {
        ...
        if (hw*2 >= m_worldSize.x) cp.x = m_worldSize.x/2; else { if(cp.x<hw) cp.x=hw; if(cp.x>m_worldSize.x-hw) cp.x=m_worldSize.x-hw; }
        if (hh*2 >= m_worldSize.y) cp.y = m_worldSize.y/2; else { if(cp.y<hh) cp.y=hh; if(cp.y>m_worldSize.y-hh) cp.y=m_worldSize.y-hh; }
    ...
    if (!m_joven.estaDormido() && m_cercaJoven && !m_tictactoeMinigame.isActive() && !m_roosterHuntMinigame.isActive() && !m_dientesObtenidos && !m_bloquearR) {
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::R)) {
            if (!rTictactoePresionado) { rTictactoePresionado = true; m_tictactoeMinigame.activate(); }
        } else rTictactoePresionado = false;
    }
    ...
    if (m_dientesObtenidos && m_abuelita.estaNormal() && m_cercaAbuelita && !m_llaveObtenida) {
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::R)) {
            if (!rAbuelitaPresionado) { rAbuelitaPresionado = true; m_abuelita.sonreir(); m_llaveObtenida = true; mostrarMensaje("Llave obtenida!", 3.f, sf::Color::Green); }
        } else rAbuelitaPresionado = false;
    }
    ...
    if (!inv || !inv->isOpen()) {
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::W) || sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Up)) m.y -= 1.f;
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::S) || sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Down)) m.y += 1.f;
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::A) || sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Left)) m.x -= 1.f;
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::D) || sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Right)) m.x += 1.f;
        if (m.x != 0.f || m.y != 0.f) { float l = std::sqrt(m.x*m.x + m.y*m.y); m /= l; }
        m_player.move(m, dt);
    }
    m_player.update(dt);
    ...
    for (const auto& obj : m_mapaFisico) {
        if (m_player.getHurtbox().findIntersection(obj.getBounds()).has_value()) { m_player.setPosition(posAnterior.x, posAnterior.y); break; }
    }
    ...
    if (m_player.getHurtbox().findIntersection(gc).has_value()) m_player.setPosition(posAnterior.x, posAnterior.y);
    ...
    if (m_player.getHurtbox().findIntersection(jc).has_value()) m_player.setPosition(posAnterior.x, posAnterior.y);
    ...
    if (m_player.getHurtbox().findIntersection(ac).has_value()) m_player.setPosition(posAnterior.x, posAnterior.y);
    ...
    if (!m_mostrarTutorial && !m_mostrarTutorialPorTecla) {
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::Key::Escape)) { if(!esc) { esc=true; game->pushState(std::make_unique<PauseState>(window, game)); } }
        else esc = false;
    }
}
...
void Nivel6State::draw()
{
    ...
    if (!m_joven.estaDormido() && m_cercaJoven && !m_tictactoeMinigame.isActive() &&
        ...
        m_textoInteraccion && m_fontLoaded)
    {
        m_textoInteraccion->setString("Presiona R para jugar la vieja");
        sf::FloatRect textBounds = m_textoInteraccion->getLocalBounds();
        m_textoInteraccion->setOrigin(sf::Vector2f(textBounds.size.x / 2.f, textBounds.size.y / 2.f));
        m_textoInteraccion->setPosition(sf::Vector2f(window->getSize().x / 2.f, window->getSize().y - 70.f));
        window->draw(*m_textoInteraccion);
    ...
    {
        m_textoInteraccion->setString("Ya tienes los dientes, ve con la abuelita");
        sf::FloatRect textBounds = m_textoInteraccion->getLocalBounds();
        m_textoInteraccion->setOrigin(sf::Vector2f(textBounds.size.x / 2.f, textBounds.size.y / 2.f));
        m_textoInteraccion->setPosition(sf::Vector2f(window->getSize().x / 2.f, window->getSize().y - 70.f));
        window->draw(*m_textoInteraccion);
