...
Player::Player() : m_speed(300.0f), m_position(600, 300), m_currentAnimation(nullptr), m_lastDirection(0, 1) {
    ...
    // NO añadimos items por defecto - el inventario empieza vacío
}
...
void Player::update(float dt) {
    ...
    if (m_inventory) m_inventory->update(dt);
}
...
sf::FloatRect Player::getHurtbox() const {
    ...
    float hurtboxWidth = fullBounds.size.x * 0.7f;
    float hurtboxHeight = fullBounds.size.y * 0.30f;
...
void Player::drawHurtbox(sf::RenderWindow& window) const {
    ...
    hurtboxShape.setFillColor(sf::Color(0, 255, 0, 150));
